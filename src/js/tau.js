	...
	vList.setListItemUpdater(function(elListItem, newIndex)
			{  
			   elListItem.innerHTML = '<a href="program.html" ' +
			   	...
			   	'class="btn-text ui-btn ui-inline">' + data[newIndex] + '</a>';
			});
	...
	if (!currentExercise.History)
	{
		...
		currentExercise.History.forEach(function(item, index) {
			if (item.date > maxDate) { 
				maxDate = item.date;
				lastHistory = item;
				populateLasts(lastHistory.weight, lastHistory.reps, lastHistory.sets);
			}
		})
	}	
...
function logTest() {
	...
	var entry = { weight: 65, reps: 8, sets: 2, date: Date.now() };
	if (!currentExercise.History) {
...
function navigateExercise(id) {
	...
	console.log("exercise found:" + currentExercise);
